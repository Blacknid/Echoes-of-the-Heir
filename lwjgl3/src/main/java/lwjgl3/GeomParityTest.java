...
/**
 * DESKTOP-ONLY dev test: proves {@code gfx.geom} reproduces {@code java.awt.geom} collision
 * results, so the migration doesn't shift any Tiled collision shapes. Imports java.awt.geom
 * deliberately (it's a desktop-side cross-check, never shipped in core/Android).
 *
 * Run:  ./gradlew :lwjgl3:run -PmainClass=lwjgl3.GeomParityTest
 * (or temporarily set application.mainClass). Prints a PASS/FAIL summary with mismatch counts.
 *
 * Tolerance: a tiny boundary band is allowed because flattened ellipses/rotations differ by
 * sub-pixel amounts from java.awt's curve flattening — collision uses integer tile cells, so
 * sub-pixel boundary disagreements are irrelevant in practice and excluded from the band.
 */
